# Process All Images with Black Background
# Processes all formatted and rotated images to replace white backgrounds with black

param(
    [string]$SourceDir = "data/processed",
    [string]$OutputDir = "data/processed/all_black_bg",
...
# Main script
Write-Host "=== Process All Images with Black Background ===" -ForegroundColor Cyan
...
# Get all PNG images (excluding black_bg subdirectory)
...
foreach ($img in $images) {
    ...
    Write-Host "[$current/$total - $percent%] Processing: $($img.Name)" -ForegroundColor Gray -NoNewline
